...
def find_semantic_split(text: str, mid: int) -> int:
    """
    尋找最適當的中文語法斷句點。
    從中間點向左右各 12 個字元擴大掃描，精準定位弱標點、關係詞、助詞與虛詞。
    """
    BREAK_AFTER_CHARS  = set('的之與和或而及在於後前中')
    BREAK_BEFORE_CHARS = set('是被將以就要會')
    PUNCT_SPLIT_CHARS  = set('，、。！？；：殘﹐,.;!?')  # 標點符號具有最高優先切分權
    
    best_idx = mid
    best_score = -1
    
    # 擴大掃描半徑至 12 個字，以便在較長的句子中精準找到逗號
    for offset in range(0, 13):
        for sign in [-1, 1]:
            ...
            if 1 <= idx < len(text):
                ...
                score = 0
                if prev_char in PUNCT_SPLIT_CHARS:
                    score = 4       # 極高優先級：直接切分在標點符號（如逗號）之後
                elif prev_char in BREAK_AFTER_CHARS:
...
def smart_wrap(text: str, font, max_w: int, draw) -> list:
    ...
    # 逗號「，」與頓號「、」被視為弱標點，在此階段不強迫斷開，從而保留「好，」等短詞與後續文字的緊密性
...
def generate_card(row, row_idx, zf, zip_index,
    ...
    if custom_text is not None:
        ...
        # 將安全寬度上限提高至 880，以完整容納 15-16 個字而免遭無謂切分
